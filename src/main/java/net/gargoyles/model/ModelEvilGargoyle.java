...
public class ModelEvilGargoyle extends ModelBase {
    ...
    {
    	this.Torso.setRotationPoint(0.0F, 10.0F, 0.0F);
    	this.setRotateAngle(RightWing1, 0.35F, 0.35F, 0.35F);
    	this.setRotateAngle(LeftWing1, -0.35F, 2.8F, -0.35F);
    	this.setRotateAngle(RightWing2, 0.0F, 0.0F, -0.175F);
    	this.setRotateAngle(LeftWing2, 0.0F, 0.0F, -0.175F);
        ...
        boolean flag = entityirongolem.world.getBlockState(new BlockPos(MathHelper.floor(entityirongolem.posX), MathHelper.floor(entityirongolem.getEntityBoundingBox().minY - 0.5D), MathHelper.floor(entityirongolem.posZ))).getBlock() == entityirongolem.getFavoriteBlockToPerch();
        if (i > 0)
        {
            this.RightArm.rotateAngleX = -2.0F + 1.5F * this.func_78172_a((float)i - p_78087_6_, 10.0F);
            this.LeftArm.rotateAngleX = -2.0F + 1.5F * this.func_78172_a((float)i - p_78087_6_, 10.0F);
        }
    	...
    	{
    	RightWing1.rotateAngleY += MathHelper.cos(p_78087_3_ * 0.4F) * 0.5F;
    	LeftWing1.rotateAngleY -= MathHelper.cos(p_78087_3_ * 0.4F) * 0.5F;
    	RightWing2.rotateAngleY += MathHelper.cos(p_78087_3_ * 0.4F - 1.5F);
    	LeftWing2.rotateAngleY -= MathHelper.cos(p_78087_3_ * 0.4F - 1.5F);
        ...
        this.Torso.rotationPointY = 16F;
        this.Torso.rotateAngleX = 1.25F;
        this.Head.rotateAngleX -= 1.25F;
    	}
    	else
            this.Torso.rotateAngleX = !entityirongolem.onGround ? 1.25F : 0F;
    }
